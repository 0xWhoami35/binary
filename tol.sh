...
init_vars()
{
        # /dev/shm is already set as HOME, no need to change
        echo "HOME is set to /dev/shm"
}
if [ -n "$MINI_PORT" ]; then
...
bashrc() {
    # Check if .bashrc exists in /dev/shm
    if [[ ! -f "$HOME/.bashrc" ]]; then
        touch "$HOME/.bashrc" || {
            echo -e "${R}ERROR:${N} Failed to create .bashrc in $HOME"
            return 1
        }
    fi
...
profile() {
    # Check if .profile exists in /dev/shm
    if [[ ! -f "$HOME/.profile" ]]; then
        touch "$HOME/.profile" || {
            echo -e "${R}ERROR:${N} Failed to create .profile in $HOME"
            return 1
        }
    fi
...
user_install() {
    ...
    mkdir -p "$TARGET_DIR" || {
        echo -e "${R}ERROR:${N} Could not create directory in /dev/shm"
        return 1
    }
